...
// --- FlixWorld Hicine Database Adapter & Secret Key Config ---
const HICINE_CONFIG = {
  apiBase: 'https://api.hicine.sbs',
  apiKey: 'hicine_website_secret_2025_exi9epdmrns',
  headers: {
...
const bakFile = path.resolve(__dirname, '..', 'js', 'index-CQL8lqua.js.bak');
const cleanContent = fs.readFileSync(bakFile, 'utf8');

const targetStr = 'F0=s=>s?.length?s.map(v=>W0[v]).filter(Boolean):[],Og=async()';
const endStr = 'return E},Qg=';

const startIdx = cleanContent.indexOf(targetStr);
const endIdx = cleanContent.indexOf(endStr, startIdx);

if (startIdx === -1 || endIdx === -1) {
  console.error('Target anchors not found!');
  process.exit(1);
}

const prefix = cleanContent.substring(0, startIdx) + 'F0=s=>s?.length?s.map(v=>W0[v]).filter(Boolean):[];';
const suffix = cleanContent.substring(endIdx + 'return E},'.length);

const finalContent = prefix + hicineAdapterCode + newServices + suffix;

targetFiles.forEach(file => {
  fs.writeFileSync(file, finalContent, 'utf8');
  console.log('✅ Correctly patched & saved:', file);
});
