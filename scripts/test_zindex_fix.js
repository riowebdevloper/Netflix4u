...
async function run() {
  ...
  ws.addEventListener('open', async () => {
    ...
    // Test setting zIndex to 100 on desktop
    const testDesktop = await evalExpr(`(() => {
      const btn = document.querySelector('a[aria-label="Go back"]');
      btn.parentElement.style.zIndex = '100';
      btn.style.zIndex = '100';
      const rect = btn.getBoundingClientRect();
      const el = document.elementFromPoint(rect.left + rect.width / 2, rect.top + rect.height / 2);
      return {
        isInsideBtn: btn.contains(el),
        elTag: el ? el.tagName : null,
        elClass: el ? el.className : null
      };
    })()`);
    console.log('Desktop with zIndex 100:', testDesktop);
    ...
    const testMobile = await evalExpr(`(() => {
      const btn = document.querySelector('a[aria-label="Go back"]');
      btn.parentElement.style.zIndex = '100';
      btn.style.zIndex = '100';
      const rect = btn.getBoundingClientRect();
    ...
    console.log('Mobile with zIndex 100:', testMobile);
